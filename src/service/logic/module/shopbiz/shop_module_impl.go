...
func (m *ShopModule) BuyGoods(shopId, goodsId int32, num int32) ([]*pb.Item, errorpb.ERROR) {
	...
	goldConfID := strconv.Itoa(configdoc.COIN_GOLD)
	cost := []*pb.Item{{ConfId: goldConfID, Num: int64(conf.Price) * int64(num)}}
	if _, code := m.ctx.Item().SubItems(cost, common.BuildReason(common.ReasonShop.Id, "", "shop buy")); code != errorpb.ERROR_SUCCESS {
		return nil, code
	}
	rewards := []*pb.Item{{ConfId: goldConfID, Num: int64(conf.Gold+conf.GiveGold) * int64(num)}}
...
func (m *ShopModule) shopConf(goodsId int32) *docpb.Gold_shopGoldShop {
	if m.ctx == nil || m.ctx.Doc() == nil || m.ctx.Doc().TbGoldShop == nil {
		return nil
	}
	return m.ctx.Doc().TbGoldShop.Get(goodsId)
}
